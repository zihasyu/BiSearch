...
./BiSearch -i $path -c 4 -m 5 -n $num   >BreakdownChunks$name.txt
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
sudo rm -r restoreFile
sudo mkdir restoreFile

./BiSearch -i $path -c 1 -m 3 -n $num -R 1 >OdessFastCDC$name.txt
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
sudo rm -r restoreFile
sudo mkdir restoreFile

./BiSearch -i $path -c 1 -m 0 -n $num -R 1 >Dedup$name.txt
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
sudo rm -r restoreFile
sudo mkdir restoreFile
