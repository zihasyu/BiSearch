...
run_RAW(){
  ...
  local name=$2
  local num=$3
./BiSearch -i $path -c 6 -m 0 -n $num  >Dedup_Skip$name.txt
sudo rm -r mTarFile
sudo mkdir mTarFile
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
./BiSearch -i $path -c 7 -m 0 -n $num  >Dedup_NoSkip$name.txt
sudo rm -r mTarFile
sudo mkdir mTarFile
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
./BiSearch -i $path -c 6 -m 3 -n $num  >Odess_Skip$name.txt
sudo rm -r mTarFile
sudo mkdir mTarFile
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
./BiSearch -i $path -c 7 -m 3 -n $num  >Odess_NoSkip$name.txt
sudo rm -r mTarFile
sudo mkdir mTarFile
sudo rm Containers/*
sudo echo 3 > /proc/sys/vm/drop_caches
